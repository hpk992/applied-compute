...
export default function ProfileCard({
  ...
  const borderClasses = isLast
    ...
    : 'border-t-2 border-l-2 border-r-2 border-foreground';

  return (
    <div className={`flex items-center justify-between p-6 ${borderClasses} ${className}`}>
      <div className="flex flex-row">
        <div className="flex w-[160px] flex-col">
          <h3 className="text-foreground font-second text-sm font-bold">{name}</h3>
          <p className="text-foreground font-second text-sm">{title}</p>
        </div>
